...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our Relay account',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    const accountName = result?.accountName ?? 'our Relay account';
    const message = `Join ${accountName} on Relay using this link (valid for ${result?.expiresInDays} days): ${url}`;
